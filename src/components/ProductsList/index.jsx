...
//     const isEnglish = pathname.includes('/en/');
//     const productPath = isEnglish ? `/en/products/${encodeURIComponent(item.slug)}` : `/products/${encodeURIComponent(item.slug)}`;
...
//   const { locale, t } = useLocale();
//   const pathname = usePathname();
//   const isEnglish = pathname?.includes('/en') || false; // استفاده از Optional Chaining
//   const title = product.title;
//   const productPath = isEnglish ? `/en/products/${encodeURIComponent(product.slug)}` : `/products/${encodeURIComponent(product.slug)}`;
...
//       const params = new URLSearchParams();
//       if (slug) params.set('category', slug);
//       const newPath = pathname?.includes('/en') ? '/en/products' : '/products'; // استفاده از Optional Chaining
//       router.push(`${newPath}?${params.toString()}#products`);
//     },
//     [router, pathname],
//   );
...
//     <section
//       id="products"
//       dir={pathname?.includes('/en') ? 'ltr' : 'rtl'} // استفاده از Optional Chaining
//       className="min-h-screen bg-white dark:bg-slate-900 text-slate-900 dark:text-white pt-12"
//     >
//       {!isOnline && (
//         <div className="fixed top-0 inset-x-0 bg-orange-500 text-white p-4 text-center z-50">
...
//           <div className="flex items-center text-yellow-600 dark:text-yellow-400 text-sm">
//             <AlertTriangle size={16} />
//             <span className={pathname?.includes('/en') ? 'ml-2' : 'mr-2'}> {/* استفاده از Optional Chaining */}
//               {t('search.caps_lock_warning')}
//             </span>
//           </div>
//         )}
